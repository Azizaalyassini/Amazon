...
public class IOSController implements ControllerInterface {
    ...
    public void startSession(String udid, String deviceName, String platformVersion, int systemPort) throws MalformedURLException {
        String appName;
        if (System.getProperty("appType").equalsIgnoreCase("agent")) {
            appName = "Agent.ipa";
        } else {
            appName = "Retail.ipa";
        }
        ...
        ThreadLocalDriver.setTLDriver(new IOSDriver(new URL("http://0.0.0.0:4723/wd/hub"), options));
        changeContext();
    }
